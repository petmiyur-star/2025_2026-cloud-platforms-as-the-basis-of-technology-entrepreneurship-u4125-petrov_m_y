...
gcloud run deploy petrov-custom --image gcr.io/cloud-platforms-as-the-basis/petrov-custom-hello --platform managed --region europe-west1 --allow-unauthenticated --port 8080
...
Через Logs Explorer выполнен запрос по ресурсу cloud\_run\_revision и сервису petrov-custom.
...
Через вкладку Traffic (Manage traffic) выполнено разделение трафика:

\- 80% на старую ревизию (порт 8080)

\- 20% на новую ревизию (порт 8090)

Затем трафик возвращён на 100% на новую ревизию.

Скриншот 6: окно управления трафиком с распределением 80%/20%.

Скриншот 7: окно управления трафиком с 100% на новой ревизии.
...
Скриншот 8: подтверждение удаления в консоли.
